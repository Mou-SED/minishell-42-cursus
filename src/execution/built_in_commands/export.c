...
/*   Updated: 2022/08/20 23:15:30 by zaabou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{ 
    ...
    else
    {
        while (node->cmd_node->cmd_table[i])
        {
            ...
            {
                if (exist_var(node->cmd_node->cmd_table[i], *(node->cmd_node->m_env)) == false)
                    export_varible(node->cmd_node->cmd_table[i], node->cmd_node->m_env);
            }
            i++;
        }
    }
